...
def test_scan_item_no_range(dynamodb):
    ...
    results = BaseModel.scan(filter_condition=A("item_id").eq("hello:world"))
...
def test_scan_item_with_range(dynamodb):
    ...
    results = RangeKeyModel.scan(filter_condition=A("item_id").eq("hello:world"))
...
def test_scan_gsi_with_filter_succeeds(dynamodb):
    _save_items(GSIModel, add_count=50)
...
def test_scan_gsi_with_filter_raises_invalid_state(dynamodb):
    _save_items(RangeKeyModel, add_count=50)
